...
+function ($) {
  ...
  Carousel.prototype.slide = function (type, next) {
    ...
    if (this.$indicators.length) {
      ...
      var $nextIndicator = $(this.$indicators.children('li')[this.getItemIndex($next)])
...
jQuery(document).ready(function ($) {
  // code here
    var test = 0;
  		...
  		// Toggle visibility for the Search Field
		$('.search-toggle').click (function(){
			$(".search-overlay").animate({width:'toggle'},150);
            ...
            $(document).keydown(function(e) {
              if (e.which == 32) {
                if($(document).scrollTop() > 500){
                  ...
                  setTimeout(function(){window.scrollTo(window.scrollX, window.scrollY -220)}, 200);
                }
              }
            });
		
		// Change homepage body class once more breaking stories have been exposed
		$('.current-breaking .tg').click (function(){
			$('body').removeClass('breaking-closed');
			$('body').addClass('breaking-expanded');
		});
		

		// Show archive filter options
		$('.archive-filters-toggle').click (function(){
			$('.archive-filters-options').slideToggle(400);
			$(this).toggleClass('open');
		});
		...
		// Show footnote
		$('.footnote-marker').click (function(){
			$(this).closest('.footnote').find('.footnote-text').fadeToggle(400);
		});
		// Hide footnote
		$('.footnote-text').click (function(){
			$(this).closest('.footnote').find('.footnote-text').fadeToggle(400);
		});
		
		
		// Show Print PDF Tooltip
		$('.print-link').hover(
		  function() {
		    $('.print-tooltip').fadeIn();
		  }, function() {
		    $('.print-tooltip').fadeOut();
		  }
		);
		
		// Customize twitter share link for each article
		$('.icon-twitter').click(function() {
			  
		    var current_href = $(location).attr('href');
			var current_title = $(document).attr('title');	
			var encoded_title = encodeURIComponent(current_title);	
			var twitter_string = 'https://twitter.com/intent/tweet?url=' + current_href + '&text=' + encoded_title;
			
			$('.icon-twitter').attr("href", twitter_string);
		});
		...
		// Homepage Breaking Scroll Fade Out
		
		$( window ).scroll(function() {
			if ($('body.page-front.breaking-closed').length) {
				var scroll_amount = $(window).scrollTop();
				$( '.current-breaking' ).css( 'opacity', 1-(scroll_amount/600) );
				$( 'header.affix-top' ).css( 'opacity', 1-(scroll_amount/600) );
			}
		});
		
		
		
		// Insert placeholder image for empty images within various views
		$('.teaser-image.empty').append( '<img src="/sites/all/themes/symmetry2015/images/emptyImage.png">' );
		$('.teaser-image.empty').removeClass( 'empty' ).addClass( 'default' );
		
		// Drupal behaviors for AJAX-related views events		
		...
		//Add url parameter to facebook sharing
		var a_href = $('.icon-facebook').attr('href');
		$('.icon-facebook').attr('href', a_href+'?u='+window.location.href);	
		
		
		// Freezeframe for Gifs
		// Add default freezeframe classes to paragarph gifs
		$('.animated-gif img, .timeline-gif img').addClass('freezeframe freezeframe-responsive');


		$(function() {
		  // Instantiate freezeframe effect	
		  ff = new freezeframe({'animation_play_duration': Infinity,
		  'non_touch_device_trigger_event': 'click'}).freeze();
		
		  // play animated gifs on inital page scroll
		  $(window).scroll(function() {
